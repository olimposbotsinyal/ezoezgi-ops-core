...
| B031 | Gerçek TR-EN çeviri/NLU entegrasyonu | Yüksek | **Kısmen tamamlandı — ilk canlı ölçüm FAIL, kök neden teşhis edildi** | Faz 1 | Bridge Agent | Adaptör + bayrak + quality gate (golden set + `tools/eval_nlu.py` + eşikler) tamam. **2026-08-13T00:01:19Z: Ollama winget ile kuruldu (`llama3:latest`), ilk canlı ölçüm çalıştı** (`reports/nlu_eval_20260813.md`) — 5 kriterden 4'ü FAIL: intent_accuracy %30 (eşik ≥%90), entity_match %0 (eşik ≥%85), fallback_rate %100 (eşik ≤%5), p95 latency 6.06s (eşik ≤2.5s); yalnızca parse_error_rate PASS. **Kök neden:** `services/tr-en-bridge/src/model_client.py::DEFAULT_TIMEOUT_SECONDS=2.0s`, ölçülen gerçek CPU inference gecikmesinden (~6s) çok kısa — her istek timeout'a uğrayıp mock fallback'e düşüyor; `intent_accuracy=%30` model performansını değil, golden set'teki `UNKNOWN` oranıyla (15/50) rastlantısal örtüşmeyi yansıtıyor. Bu bir model-kalite sorunu değil, bir **konfigürasyon** sorunu. Sonraki adım: timeout'u yükseltip (ör. 15-30s) yeniden ölçmek — bkz. `docs/RUNBOOK.md` "Değerlendirme koşu kayıtları" |
