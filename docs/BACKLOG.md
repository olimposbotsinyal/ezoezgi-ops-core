...
| B036 | Ollama Windows `0xc0000005` (`llama-server` crash) araştırılması | **Yüksek** | Açık — **4 deney denendi, hiçbiri düzeltmedi** | Faz 1 | Bridge Agent | 2026-08-13T00:38:34Z stabilizasyon kapısı koşuldu (`reports/runtime_diag_20260813T003834Z/`): (A) baseline, (B) `OLLAMA_NUM_GPU=0` + servis restart, (C) temiz `rm`+`pull` (sha256 doğrulandı), (D) farklı model (`llama3`) — **4/4 aynı `0xc0000005` ile çöktü**. Model boyutu/ailesi, model dosya bütünlüğü ve (denenen) GPU ayarından bağımsız. **Önemli çekince:** (B)'de `OLLAMA_NUM_GPU=0`'a rağmen sunucu logu hâlâ `Vulkan0: Quadro RTX 3000` cihazını listeledi — bu ayar Vulkan/GPU algılamasını tam kapatmamış olabilir, GPU/Vulkan sürücü etkileşimi kesin ekarte edilemedi. 50 çağrılık hard gate hiç koşulmadı (tekli testlerin 0/4 başarı oranı zaten FAIL'i kanıtlıyordu). **Kabul kriteri (değişmedi):** B031 quality gate'i yeniden koşmadan önce 50/50 başarılı inference çağrısı. **Rafine edilmiş sonraki adımlar:** (1) Vulkan/GPU'yu GERÇEKTEN devre dışı bırakan doğru ortam değişkenini bulmak (`OLLAMA_NUM_GPU=0` yetersiz görünüyor — `GGML_VK_VISIBLE_DEVICES=""` veya farklı bir bayrak denenebilir), (2) Visual C++ Redistributable / Vulkan sürücüsünü güncellemek, (3) farklı bir Ollama sürümü kurmak, (4) Ollama'nın GitHub issue'larında `0xc0000005` aramak |
