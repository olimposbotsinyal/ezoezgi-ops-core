...
| B031 | Gerçek TR-EN çeviri/NLU entegrasyonu | Yüksek | **Kısmen tamamlandı** | Faz 1 | Bridge Agent | `services/tr-en-bridge/src/ollama_nlu.py` + `bridge.py` `NLU_PROVIDER` bayrağı ile eklendi (varsayılan hâlâ `mock`, güvenli). Kalan iş: prompt/entity çıkarımının gerçek Ollama modeliyle uçtan uca kalite testi (bu ortamda Ollama kurulu değil, yalnızca mock-fallback yolu doğrulandı) — bkz. `docs/RUNBOOK.md` "Faz 1 — Ollama NLU Entegrasyonu" |
